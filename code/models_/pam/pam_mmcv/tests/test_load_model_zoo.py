...
import mmcv
from pam_mmcv.runner.checkpoint import (DEFAULT_CACHE_DIR, ENV_MMCV_HOME,
...
@patch('pam_mmcv.__path__', [osp.join(osp.dirname(__file__), 'data/')])
...
def test_default_mmcv_home():
    ...
    assert _get_mmcv_home() == os.path.expanduser(
        os.path.join(DEFAULT_CACHE_DIR, 'pam_mmcv'))
...
@patch('pam_mmcv.__path__', [osp.join(osp.dirname(__file__), 'data/')])
@patch('pam_mmcv.runner.checkpoint.load_from_http', load_from_http)
@patch('pam_mmcv.runner.checkpoint.load_url', load_url)
