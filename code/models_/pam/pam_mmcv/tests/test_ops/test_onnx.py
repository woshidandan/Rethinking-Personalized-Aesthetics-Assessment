...
def test_grid_sample(mode, padding_mode, align_corners):
    from pam_mmcv.onnx.symbolic import register_extra_symbolics
    opset_version = 11
...
def test_bilinear_grid_sample(align_corners):
    from pam_mmcv.ops.point_sample import bilinear_grid_sample
...
def test_nms():
    from pam_mmcv.ops import get_onnxruntime_op_path, nms
...
def test_softnms():
    from pam_mmcv.ops import get_onnxruntime_op_path, soft_nms
...
def test_roialign():
    try:
        from pam_mmcv.ops import get_onnxruntime_op_path, roi_align
...
def test_roialign_rotated():
    try:
        from pam_mmcv.ops import get_onnxruntime_op_path, roi_align_rotated
    except (ImportError, ModuleNotFoundError):
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason='test requires GPU')
def test_roipool():
    from pam_mmcv.ops import roi_pool
...
def test_rotated_feature_align():
    ...
    try:
        from pam_mmcv.ops import get_onnxruntime_op_path, rotated_feature_align
    except (ImportError, ModuleNotFoundError):
...
def test_corner_pool(mode, opset=11):
    ...
    from pam_mmcv.ops.corner_pool import CornerPool
...
def test_cummax_cummin(key, opset=11):
    ...
    # register custom op `pam_mmcv::cummax` and `pam_mmcv::cummin`
    from pam_mmcv.onnx.symbolic import register_extra_symbolics
...
def test_modulated_deform_conv2d():
    try:
        from pam_mmcv.ops import ModulatedDeformConv2d, get_onnxruntime_op_path
    except (ImportError, ModuleNotFoundError):
...
def test_deform_conv2d(threshold=1e-3):
    try:
        from pam_mmcv.ops import DeformConv2d, get_onnxruntime_op_path
    except (ImportError, ModuleNotFoundError):
