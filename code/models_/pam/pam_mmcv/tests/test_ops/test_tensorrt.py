...
try:
    from pam_mmcv.tensorrt import (TRTWrapper, is_tensorrt_plugin_loaded, onnx2trt,
                               save_trt_engine)
except ImportError:
    pytest.skip(
...
if not is_tensorrt_plugin_loaded():
    pytest.skip(
        'Test requires to complie TensorRT plugins in pam_mmcv',
...
def test_roialign():
    try:
        from pam_mmcv.ops import RoIAlign
...
def test_batched_nms():
    try:
        import mmcv
        from pam_mmcv.ops import batched_nms
    except (ImportError, ModuleNotFoundError):
        pytest.skip('test requires compilation')
...
def test_deform_conv():
    try:
        from pam_mmcv.ops import DeformConv2dPack
    except (ImportError, ModuleNotFoundError):
        pytest.skip('test requires compilation')
...
def test_modulated_deform_conv(with_bias):
    try:
        from pam_mmcv.ops import ModulatedDeformConv2dPack
    except (ImportError, ModuleNotFoundError):
        pytest.skip('test requires compilation')
...
def test_corner_pool(mode):
    try:
        from pam_mmcv.ops import CornerPool
    except (ImportError, ModuleNotFoundError):
        pytest.skip('test requires compilation')

    opset = 11
    # register custom op `pam_mmcv::MMCVCornerPool`
    from pam_mmcv.onnx.symbolic import register_extra_symbolics
    register_extra_symbolics(opset)
