...
from pam_mmcv import get_logger, print_log
...
def test_print_log_logger(caplog):
    print_log('welcome', logger='pam_mmcv')
    assert caplog.record_tuples[-1] == ('pam_mmcv', logging.INFO, 'welcome')

    print_log('welcome', logger='pam_mmcv', level=logging.ERROR)
    assert caplog.record_tuples[-1] == ('pam_mmcv', logging.ERROR, 'welcome')
