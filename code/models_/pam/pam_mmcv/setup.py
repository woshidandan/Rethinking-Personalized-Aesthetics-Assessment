...
def get_version():
    version_file = 'pam_mmcv/version.py'
...
def get_extensions():
    ...
    if os.getenv('MMCV_WITH_TRT', '0') != '0':
        ...
        include_path = os.path.abspath('pam_mmcv/ops/csrc/common/cuda')
        include_trt_path = os.path.abspath('pam_mmcv/ops/csrc/tensorrt')
        include_dirs.append(include_path)
        include_dirs.append(include_trt_path)
        include_dirs.append(os.path.join(tensorrt_path, 'include'))
        include_dirs += include_paths(cuda=True)

        op_files = glob.glob('pam_mmcv/ops/csrc/tensorrt/plugins/*')
    ...
    if EXT_TYPE == 'parrots':
        ...
        define_macros = []
        include_dirs = []
        op_files = glob.glob('pam_mmcv/ops/csrc/pytorch/cuda/*.cu') + \
                   glob.glob('pam_mmcv/ops/csrc/pytorch/cpu/*.cpp') + \
                   glob.glob('pam_mmcv/ops/csrc/parrots/*.cpp')
        include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common'))
        include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common/cuda'))
        cuda_args = os.getenv('MMCV_CUDA_ARGS')
        extra_compile_args = {
        ...
        if is_rocm_pytorch or torch.cuda.is_available() or os.getenv(
            ...
            cuda_args = os.getenv('MMCV_CUDA_ARGS')
            extra_compile_args['nvcc'] = [cuda_args] if cuda_args else []
            op_files = glob.glob('pam_mmcv/ops/csrc/pytorch/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/cpu/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/cuda/*.cu') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/cuda/*.cpp')
            extension = CUDAExtension
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/pytorch'))
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common'))
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common/cuda'))
            ...
            op_files = glob.glob('pam_mmcv/ops/csrc/pytorch/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/cpu/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/mlu/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/common/mlu/*.mlu')
            extension = MLUExtension
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common'))
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common/mlu'))
            ...
            op_files = glob.glob('pam_mmcv/ops/csrc/pytorch/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/cpu/*.cpp') + \
                       glob.glob('pam_mmcv/ops/csrc/common/mps/*.mm') + \
                       glob.glob('pam_mmcv/ops/csrc/pytorch/mps/*.mm')
            extension = CppExtension
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common'))
            include_dirs.append(os.path.abspath('pam_mmcv/ops/csrc/common/mps'))
        else:
    ...
    if EXT_TYPE == 'pytorch' and os.getenv('MMCV_WITH_ORT', '0') != '0':
        ...
        include_path = os.path.abspath('pam_mmcv/ops/csrc/onnxruntime')
        include_dirs.append(include_path)
        include_dirs.append(os.path.join(ort_path, 'include'))

        op_files = glob.glob('pam_mmcv/ops/csrc/onnxruntime/cpu/*')
        if onnxruntime.get_device() == 'GPU' or os.getenv('FORCE_CUDA',
                                                          '0') == '1':
            define_macros += [('MMCV_WITH_CUDA', None)]
            cuda_args = os.getenv('MMCV_CUDA_ARGS')
            extra_compile_args['nvcc'] = [cuda_args] if cuda_args else []
            op_files += glob.glob('pam_mmcv/ops/csrc/onnxruntime/gpu/*')
            include_dirs += include_paths(cuda=True)
            library_dirs += library_paths(cuda=True)
        else:
