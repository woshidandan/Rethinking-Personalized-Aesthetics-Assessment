...
from pam_mmcv.fileio import FileClient
from pam_mmcv.utils import is_filepath, is_str
...
def imread(img_or_path: Union[np.ndarray, str, Path],
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            ``pam_mmcv.use_backend()`` will be used. Default: None.
        file_client_args (dict | None): Arguments to instantiate a
            FileClient. See :class:`pam_mmcv.fileio.FileClient` for details.
            Default: None.
    ...
    Examples:
        ...
        >>> img = pam_mmcv.imread(img_path)
        >>> img = pam_mmcv.imread(img_path, flag='color', channel_order='rgb',
        ...     backend='cv2')
        >>> img = pam_mmcv.imread(img_path, flag='color', channel_order='bgr',
        ...     backend='pillow')
        >>> s3_img_path = 's3://bucket/img.jpg'
        >>> # infer the file backend by the prefix s3
        >>> img = pam_mmcv.imread(s3_img_path)
        >>> # manually set the file backend petrel
        >>> img = pam_mmcv.imread(s3_img_path, file_client_args={
        ...     'backend': 'petrel'})
        >>> http_img_path = 'http://path/to/img.jpg'
        >>> img = pam_mmcv.imread(http_img_path)
        >>> img = pam_mmcv.imread(http_img_path, file_client_args={
        ...     'backend': 'http'})
    """
...
def imfrombytes(content: bytes,
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            None, the global imread_backend specified by ``pam_mmcv.use_backend()``
    ...
    Examples:
        ...
        >>>     img_buff = f.read()
        >>> img = pam_mmcv.imfrombytes(img_buff)
        >>> img = pam_mmcv.imfrombytes(img_buff, flag='color', channel_order='rgb')
        >>> img = pam_mmcv.imfrombytes(img_buff, backend='pillow')
        >>> img = pam_mmcv.imfrombytes(img_buff, backend='cv2')
    """
...
def imwrite(img: np.ndarray,
    ...
    Examples:
        ...
        >>> ret = pam_mmcv.imwrite(img, '/path/to/img.jpg')
        >>> # infer the file backend by the prefix s3
        >>> ret = pam_mmcv.imwrite(img, 's3://bucket/img.jpg')
        >>> # manually set the file backend petrel
        >>> ret = pam_mmcv.imwrite(img, 's3://bucket/img.jpg', file_client_args={
        ...     'backend': 'petrel'})
    """
