...
from pam_mmcv.runner import (HOOKS, RUNNERS, BaseRunner, EpochBasedRunner,
                         IterBasedRunner)
from pam_mmcv.utils import IS_IPU_AVAILABLE
...
class IPUBaseRunner(BaseRunner):
    ...
    Args:
        ...
        options_cfg (pam_mmcv.Config, dict): Options that will be used to compile
            and run the model.
        modules_to_record (pam_mmcv.Config, list): Index or name of modules which
            will be recorded for output. It is necessary to specify output for
            static graph of model training or inference.
        ipu_model_cfg (pam_mmcv.Config, dict): Config of model partition and
            recomputing checkpoint
        fp16_cfg (pam_mmcv.Config): Config for fp16 training.
