...
from pam_mmcv.cnn import (Conv2d, build_activation_layer, build_norm_layer,
                      constant_init, normal_init, trunc_normal_init)
from pam_mmcv.cnn.bricks.drop import build_dropout
from pam_mmcv.cnn.bricks.transformer import MultiheadAttention
from pam_mmcv.cnn.utils.weight_init import trunc_normal_
from pam_mmcv.runner import (BaseModule, ModuleList, Sequential, _load_checkpoint,
                         load_state_dict)
...
class SpatialReductionAttention(MultiheadAttention):
    ...
    pam_mmcv.cnn.bricks.transformer.
    ...
    def __init__(self,
        ...
        from pam_mmpose import digit_version, mmcv_version
        if mmcv_version < digit_version('1.3.17'):
            warnings.warn('The legacy version of forward function in'
                          'SpatialReductionAttention is deprecated in'
                          'pam_mmcv>=1.3.17 and will no longer support in the'
                          'future. Please upgrade your pam_mmcv.')
    ...
    def legacy_forward(self, x, hw_shape, identity=None):
        """multi head attention forward in pam_mmcv version < 1.3.17."""
        x_q = x
        if self.sr_ratio > 1:
            x_kv = nlc_to_nchw(x, hw_shape)
            x_kv = self.sr(x_kv)
