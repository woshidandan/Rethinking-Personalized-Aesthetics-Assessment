...
class Config:
    def get_config_fromfile(self, config_path):
        ...
        self.human_model_path = osp.join(
            self.root_dir, "common", "utils_smpler_x", "human_model_files"
        )
...
cfg_root_path = Path(__file__).parent.parent
cfg = Config()
config_path = osp.join(cfg_root_path, "main/config", "config_smpler_x_l32.py")
ckpt_path = osp.join(cfg_root_path, "pretrained_models", "smpler_x_l32.pth.tar")
cfg.get_config_fromfile(config_path)
cfg.encoder_config_file = osp.join(cfg_root_path, cfg.encoder_config_file)
cfg.encoder_pretrained_model_path = osp.join(
    cfg_root_path, cfg.encoder_pretrained_model_path
)
cfg.prepare_dirs("output/test")
cfg.update_test_config(
    "EHF", "na", shapy_eval_split=None, pretrained_model_path=ckpt_path, use_cache=False
)
