...
from utils_smpler_x.transforms import rot6d_to_axis_angle, restore_bbox
...
class Model(nn.Module):
    ...
    def get_coord(self, root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode):
        ...
        output_zero_pose = self.smplx_layer(betas=shape, body_pose=torch.zeros_like(body_pose),
                                            global_orient=torch.zeros_like(root_pose),
                                            right_hand_pose=torch.zeros_like(rhand_pose),
                                            left_hand_pose=torch.zeros_like(lhand_pose),
                                            jaw_pose=torch.zeros_like(jaw_pose), leye_pose=zero_pose,
                                            reye_pose=zero_pose, expression=expr)
        ...
        return joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_cam_wo_ra, face_cam
    ...
    def generate_mesh_gt(self, targets, mode):
        ...
        # final output
        joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_cam_wo_ra, face_cam = self.get_coord(root_pose,
                                                                                                        body_pose,
                                                                                                        lhand_pose,
                                                                                                        rhand_pose,
                                                                                                        jaw_pose, shape,
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # final output
        joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_cam_wo_ra, face_cam = self.get_coord(
            root_pose,
            body_pose,
            lhand_pose,
            rhand_pose,
        ...
        if mode == 'train':
            ...
            if getattr(cfg, 'agora_fix_global_orient_transl', False):
                ...
                if hasattr(cfg, 'smplx_orient_weight'):
                    ...
                    loss['smplx_orient'] = self.param_loss(pose, targets['smplx_pose'], meta_info['smplx_pose_valid'])[
                                           :, :3] * smplx_orient_weight

                loss['smplx_pose'] = self.param_loss(pose, targets['smplx_pose'],
            ...
            loss['joint_cam'] = self.coord_loss(joint_cam_wo_ra, targets['joint_cam'],
                                                meta_info['joint_valid'] * meta_info['is_3D'][:, None,
                                                                           None]) * smplx_kps_3d_weight
            # supervision for keypoints3d w/ ra
            loss['smplx_joint_cam'] = self.coord_loss(joint_cam, targets['smplx_joint_cam'],
                                                      meta_info['smplx_joint_valid']) * smplx_kps_3d_weight
            ...
            if not (meta_info['lhand_bbox_valid'] + meta_info['rhand_bbox_valid'] == 0).all():
                ...
                for part_name, bbox in (('lhand', lhand_bbox), ('rhand', rhand_bbox)):
                    for coord_name, trunc_name in (
                            ('joint_img', 'joint_trunc'), ('smplx_joint_img', 'smplx_joint_trunc')):
                        ...
                        targets[coord_name] = torch.cat(
                            (targets[coord_name][:, :smpl_x.joint_part[part_name][0], :], coord,
                             targets[coord_name][:, smpl_x.joint_part[part_name][-1] + 1:, :]),
                            1)
                        meta_info[trunc_name] = torch.cat(
                            (meta_info[trunc_name][:, :smpl_x.joint_part[part_name][0], :],
                             trunc,
                ...
                for part_name, bbox in (('lhand', lhand_bbox), ('rhand', rhand_bbox)):
                    ...
                    for bid in range(coord.shape[0]):
                        ...
                        if torch.sum(mask) == 0:
                            ...
                            trans.append(
                                (-coord[bid, mask, :2] + targets['joint_img'][:, smpl_x.joint_part[part_name], :][
                                                         bid, mask, :2]).mean(0))
            ...
            loss['joint_img'] = self.coord_loss(joint_img, smpl_x.reduce_joint_set(targets['joint_img']),
                                                ...
                                                meta_info['is_3D']) * net_kps_2d_weight

            loss['smplx_joint_img'] = self.coord_loss(joint_img, smpl_x.reduce_joint_set(targets['smplx_joint_img']),
                                                      smpl_x.reduce_joint_set(
                                                          meta_info['smplx_joint_trunc'])) * net_kps_2d_weight
            ...
            out['joint_origin_root_cam'] = joint_cam_wo_ra
