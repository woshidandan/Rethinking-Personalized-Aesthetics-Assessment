...
from utils_smpler_x.distribute_utils import (
    get_rank,
    is_main_process,
    time_synchronized,
    get_group_idx,
...
class Trainer(Base):
    ...
    def get_optimizer(self, model):
        ...
        optim_params = [
            {"params": normal_param, "lr": cfg.lr},  # add normal params first
            {"params": special_param, "lr": cfg.lr * cfg.lr_mult},
        ]
    ...
    def _make_batch_generator(self):
        ...
        for i in range(len(cfg.trainset_humandata)):
            trainset_humandata_loader.append(
                eval(cfg.trainset_humandata[i])(transforms.ToTensor(), "train")
            )
        ...
        if data_strategy == "concat":
            ...
            trainset_loader = MultipleDatasets(
                trainset3d_loader + trainset2d_loader + trainset_humandata_loader,
                make_same_len=False,
                verbose=True,
            )
            ...
            if len(trainset_humandata_loader) > 0:
                trainset_humandata_loader = [
                    MultipleDatasets(trainset_humandata_loader, make_same_len=False)
                ]
                valid_loader_num += 1
        ...
        if self.distributed:
            ...
            sampler_train = DistributedSampler(
                trainset_loader, world_size, rank, shuffle=True
            )
            self.batch_generator = DataLoader(
                ...
                pin_memory=True,
                persistent_workers=True if cfg.num_thread > 0 else False,
                drop_last=True,
            )
        else:
            self.batch_generator = DataLoader(
    ...
    def _make_model(self, device):
        ...
        if self.distributed:
            ...
            if hasattr(cfg, "syncbn") and cfg.syncbn:
                ...
                syncbn_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    model, process_group
                )
                model = torch.nn.parallel.DistributedDataParallel(
                    syncbn_model, device_ids=[self.gpu_idx], find_unused_parameters=True
                )
            else:
                model = torch.nn.parallel.DistributedDataParallel(
        ...
        if hasattr(cfg, "scheduler"):
            if cfg.scheduler == "cos":
                scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer, cfg.end_epoch * self.itr_per_epoch, eta_min=1e-6
                )
...
class Tester(Base):
    ...
    def _make_model(self, device):
        ...
        if not getattr(cfg, "random_init", False):
            ckpt = torch.load(
                cfg.pretrained_model_path, map_location=torch.device("cpu")
            )
            ...
            for k, v in ckpt["network"].items():
                ...
                new_state_dict[k] = v
            self.logger.warning(
                "Attention: Strict=False is set for checkpoint loading. Please check manually."
            )
...
class Demoer(Base):
    ...
    def _make_batch_generator(self, demo_scene):
        ...
        testset_loader = UBody(
            transforms.ToTensor(), "demo", demo_scene
        )  # eval(demoset)(transforms.ToTensor(), "demo")
        batch_generator = DataLoader(
            dataset=testset_loader,
            batch_size=cfg.num_gpus * cfg.test_batch_size,
            shuffle=False,
    ...
    def _make_model(self, device):
        ...
        for k, v in ckpt["network"].items():
            if "module" not in k:
                k = "module." + k
            k = (
                k.replace("module.backbone", "module.encoder")
                .replace("body_rotation_net", "body_regressor")
                .replace("hand_rotation_net", "hand_regressor")
            )
            new_state_dict[k] = v
